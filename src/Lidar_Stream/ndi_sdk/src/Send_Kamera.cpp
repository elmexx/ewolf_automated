...
public:
    NDI_Subscriber()
        : Node("camera_stream"), p_frame_buffers{nullptr, nullptr}
    {       
        frame = 0;
        ...
        NDI_send_create_desc.p_ndi_name = "Camera";
        ...
        img_frame_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/detection/object/image_raw", 10, std::bind(&NDI_Subscriber::topic_callback, this, _1));
        
        // pic_size = img_frame_->width * img_frame_->height * 3;
    }
...
private:
    ...
    {
        //unsigned int pic_size = msg->width * msg->height * 4;

        if(!p_frame_buffers[0]) {
            p_frame_buffers[0] = malloc(848*480*4);
            p_frame_buffers[1] = malloc(848*480*4);
        }
        ...
        try
        {
            ...
            // cv_ptr = cv::cvtColor(cv_ptr, cv::COLOR_RGB2YUV)
        }
        ...
        //memcpy(p_frame_buffers[frame & 1], picture, 1080 * 1080 * 3);        
